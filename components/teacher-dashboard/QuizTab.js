...
export default function QuizTab() {
  ...
  const quizScoreRows = useMemo(() => {
    ...
    return (selectedScoreSection.studentPerformance || []).map((student, index) => ({
      ...
      stars:
        student.subjectPerformance >= 90
          ? 5
          : student.subjectPerformance >= 80
            ? 4
            : student.subjectPerformance >= 70
              ? 3
              : 2,
          ...
          <article className="mt-5 overflow-visible rounded-3xl border border-slate-200 bg-white ">
            ...
            <div>
              <div className="space-y-2 px-3 py-3 sm:px-4">
                {pagedQuizScoreRows.map((student, index) => (
                  <div key={student.id} className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2">
                    <div className="flex items-center gap-3">
                      <span className="flex w-8 justify-center text-sm font-bold text-slate-500">{medalNode((safeScorePage - 1) * scorePageSize + index + 1)}</span>
                    ...
                    <div className="inline-flex items-center -space-x-1 rounded-full  px-2.5 py-1.5">
                      {Array.from({ length: student.stars }).map((_, starIndex) => (
                        <Star key={`${student.id}-star-${starIndex}`} className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                      ))}
                    </div>
                  </div>
                ))}
